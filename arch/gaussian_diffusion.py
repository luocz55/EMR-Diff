...
def get_named_eta_schedule(
    ...
    if schedule_name == 'exponential':
        ...
        sqrt_etas = np.linspace(etas_start, etas_end, num_diffusion_timesteps, dtype=np.float64)
...
class SpectralAwareDiffusion(nn.Module):
    def __init__(self, band_dim, total_steps=15, kappa=2.0):
        super().__init__()
        self.band_dim = band_dim
        self.kappa = kappa

        # 可学习的光谱权重参数
        self.spectral_weights = nn.Parameter(th.ones(1, band_dim, 1, 1))

        # 初始化噪声调度
        self.eta_schedule = self._geometric_schedule(total_steps)

    def _geometric_schedule(self, T):
        """生成基础几何进度表 (与原始ResShift一致)"""
        t = th.arange(0, T + 1)
        # η_t: 从0→1的单调递增序列
        sqrt_eta = th.sqrt(t / T)  # 线性进度
        return sqrt_eta ** 2  # η_t

    def forward_diffusion(self, x0, y0, t):
        """
        光谱感知前向扩散过程 (q(x_t | x_0, y_0))
        修改：将光谱权重应用到噪声上
        """
        # 计算联合残差 (唯一改动点)
        e0 = self.residual_calculator(x0, y0)

        # 获取当前时间步的η_t
        eta_t = self.eta_schedule[t]  # 标量值

        # 采样噪声 ε ~ N(0, I)
        noise = th.randn_like(x0)

        # 应用光谱权重 - 关键修改点
        weighted_noise = noise * self.spectral_weights

        # 核心扩散公式 (加入光谱权重)
        x_t = x0 + eta_t * e0 + self.kappa * th.sqrt(eta_t) * weighted_noise

        return x_t, weighted_noise  # 返回加权噪声用于训练

    def get_band_weights(self):
        """获取当前波段权重"""
        return self.spectral_weights.detach().cpu().numpy()
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.sf = sf
        self.band_dim = band_dim
        self.spectral_module = SpectralAwareDiffusion(
            band_dim=band_dim,
            total_steps=len(sqrt_etas),
            kappa=kappa
        )
