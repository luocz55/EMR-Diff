...
def save_checkpoint(model, epoch, data):
        model_out_path = "checkpoints/{}_{}/model_epoch_{}.pth.tar".format('EMRDIFF', data, epoch)
        state = {"epoch": epoch, "model": model}

        if not os.path.exists("checkpoints/{}_{}_{}".format('EMRDIFF', data, epoch)):
            os.makedirs("checkpoints/{}_{}_{}".format('EMRDIFF', data,epoch))
...
class ResShiftTrainer:
    ...
    def test(self):
        ...
        for step, [gt, lq1, rgb] in enumerate(tqdm(self.val_dataloader)):
            ...
            model = self.Net
            checkpoint = torch.load('/home/yaost/ResDiffusion/checkpoints/EMRDIFF_harvard/model_epoch_3.pth.tar', weights_only=False)
